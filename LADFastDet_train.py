...
class Loss(nn.Module):
    ...
    def box_eiou(self, b1, b2):
        ...
        enclose_diagonal = torch.clamp(enclose_diagonal, min=1e-6)  # 防止除零

        aspect_ratio_diff = torch.pow((b1_wh[..., 0] / torch.clamp(b1_wh[..., 1], min=1e-6)) -
                                      (b2_wh[..., 0] / torch.clamp(b2_wh[..., 1], min=1e-6)), 2)

        elastic_penalty = aspect_ratio_diff

        eiou = iou - (center_distance / torch.clamp(enclose_diagonal, min=1e-6)) + elastic_penalty
    ...
    def forward(self, l, input, targets=None):
        ...
        if n != 0:
            ...
            eiou        = self.box_eiou(pred_boxes, y_true[..., :4]).type_as(x)
            # loss_loc    = torch.mean((1 - eiou)[obj_mask] * box_loss_scale[obj_mask])
            loss_loc    = torch.mean((1 - eiou)[obj_mask])
